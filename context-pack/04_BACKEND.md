...
- llm in ("qwen","lmstudio") uses local OpenAI-compatible backend (call_qwen()).

### LLM backends (main.py)
**Local OpenAI-compatible (“qwen/lmstudio path”)**
- QWEN_BASE_URL default http://127.0.0.1:1234/v1
- QWEN_API_KEY default qwen-local
- QWEN_MODEL default mistral-nemo-base-2407
- Uses OpenAI(base_url=..., api_key=...) and chat.completions.create(...)

Note:
- inside Docker, 127.0.0.1 is the container itself. If LM Studio is on the host, set QWEN_BASE_URL to a host-reachable address (often LAN IP). On Linux, host.docker.internal is not guaranteed unless configured.
